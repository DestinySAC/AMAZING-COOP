...
Action OnFireTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype)
{
	if (MT_IsCorePluginEnabled() && bIsValidClient(victim, MT_CHECK_INDEX|MT_CHECK_INGAME|MT_CHECK_ALIVE) && damage > 0.0)
	{
		...
		{
			if ((g_esFireCache[attacker].g_iFireHitMode == 0 || g_esFireCache[attacker].g_iFireHitMode == 1) && bIsSurvivor(victim) && g_esFireCache[attacker].g_iComboAbility == 0)
			{
				if ((!MT_HasAdminAccess(attacker) && !bHasAdminAccess(attacker, g_esFireAbility[g_esFirePlayer[attacker].g_iTankTypeRecorded].g_iAccessFlags, g_esFirePlayer[attacker].g_iAccessFlags)) || MT_IsAdminImmune(victim, attacker) || bIsAdminImmune(victim, g_esFirePlayer[attacker].g_iTankType, g_esFireAbility[g_esFirePlayer[attacker].g_iTankTypeRecorded].g_iImmunityFlags, g_esFirePlayer[victim].g_iImmunityFlags))
				{
			...
			if ((damagetype & DMG_BURN) && (g_esFireCache[attacker].g_iFireAbility > 0 || g_esFireCache[attacker].g_iFireDeath > 0 || g_esFireCache[attacker].g_iFireHit > 0 || g_esFireCache[attacker].g_iFireRockBreak > 0) && g_esFireCache[attacker].g_flFireDamage > 0.0)
			{
		...
		{
			if ((!MT_HasAdminAccess(victim) && !bHasAdminAccess(victim, g_esFireAbility[g_esFirePlayer[victim].g_iTankTypeRecorded].g_iAccessFlags, g_esFirePlayer[victim].g_iAccessFlags)) || MT_IsAdminImmune(attacker, victim) || bIsAdminImmune(attacker, g_esFirePlayer[victim].g_iTankType, g_esFireAbility[g_esFirePlayer[victim].g_iTankTypeRecorded].g_iImmunityFlags, g_esFirePlayer[attacker].g_iImmunityFlags))
			{
				return Plugin_Continue;
			}
...
#endif
{
	if (bIsInfected(tank, MT_CHECK_FAKECLIENT) && g_esFireCache[tank].g_iHumanAbility != 2)
	{
		return;
	}
		...
		for (int iPos = 0; iPos < (sizeof sSubset); iPos++)
		{
			if (StrEqual(sSubset[iPos], MT_FIRE_SECTION, false) || StrEqual(sSubset[iPos], MT_FIRE_SECTION2, false) || StrEqual(sSubset[iPos], MT_FIRE_SECTION3, false) || StrEqual(sSubset[iPos], MT_FIRE_SECTION4, false))
			{
				...
				switch (type)
				{
					...
					case MT_COMBO_POSTSPAWN: vFireRange(tank, 0, 1, random, iPos);
					case MT_COMBO_UPONDEATH: vFireRange(tank, 0, 0, random, iPos);
				}
	...
	switch (mode)
	{
		case 1:
		{
			for (int iIndex = MT_GetMinType(); iIndex <= MT_GetMaxType(); iIndex++)
			{
				...
				g_esFireAbility[iIndex].g_flFireDamage = 3.0;
				...
				g_esFireSpecial[iIndex].g_flFireDamage = -1.0;
		...
		case 3:
		{
			for (int iPlayer = 1; iPlayer <= MaxClients; iPlayer++)
			{
				g_esFirePlayer[iPlayer].g_iAccessFlags = -1;
				g_esFirePlayer[iPlayer].g_iImmunityFlags = -1;
				g_esFirePlayer[iPlayer].g_flCloseAreasOnly = -1.0;
				g_esFirePlayer[iPlayer].g_iComboAbility = -1;
				g_esFirePlayer[iPlayer].g_iHumanAbility = -1;
				g_esFirePlayer[iPlayer].g_iHumanAmmo = -1;
				g_esFirePlayer[iPlayer].g_iHumanCooldown = -1;
				g_esFirePlayer[iPlayer].g_iHumanRangeCooldown = -1;
				g_esFirePlayer[iPlayer].g_iHumanRockCooldown = -1;
				g_esFirePlayer[iPlayer].g_flOpenAreasOnly = -1.0;
				g_esFirePlayer[iPlayer].g_iRequiresHumans = -1;
				g_esFirePlayer[iPlayer].g_iFireAbility = -1;
				g_esFirePlayer[iPlayer].g_iFireEffect = -1;
				g_esFirePlayer[iPlayer].g_iFireMessage = -1;
				g_esFirePlayer[iPlayer].g_flFireChance = -1.0;
				g_esFirePlayer[iPlayer].g_iFireCooldown = -1;
				g_esFirePlayer[iPlayer].g_flFireDamage = -1.0;
				g_esFirePlayer[iPlayer].g_iFireDeath = -1;
				g_esFirePlayer[iPlayer].g_flFireDeathChance = -1.0;
				g_esFirePlayer[iPlayer].g_iFireHit = -1;
				g_esFirePlayer[iPlayer].g_iFireHitMode = -1;
				g_esFirePlayer[iPlayer].g_flFireRange = -1.0;
				g_esFirePlayer[iPlayer].g_flFireRangeChance = -1.0;
				g_esFirePlayer[iPlayer].g_iFireRangeCooldown = -1;
				g_esFirePlayer[iPlayer].g_iFireRockBreak = -1;
				g_esFirePlayer[iPlayer].g_flFireRockChance = -1.0;
				...
				g_esFireTeammate[iPlayer].g_flFireDamage = -1.0;
...
#endif
{
	if ((mode == -1 || mode == 3) && bIsValidClient(admin))
	{
		if (special && specsection[0] != '\0')
		{
			...
			g_esFireTeammate[admin].g_flFireDamage = flGetKeyValue(subsection, MT_FIRE_SECTION, MT_FIRE_SECTION2, MT_FIRE_SECTION3, MT_FIRE_SECTION4, key, "FireDamage", "Fire Damage", "Fire_Damage", "damage", g_esFireTeammate[admin].g_flFireDamage, value, -1.0, 99999.0);
		...
		{
			g_esFirePlayer[admin].g_flCloseAreasOnly = flGetKeyValue(subsection, MT_FIRE_SECTION, MT_FIRE_SECTION2, MT_FIRE_SECTION3, MT_FIRE_SECTION4, key, "CloseAreasOnly", "Close Areas Only", "Close_Areas_Only", "closeareas", g_esFirePlayer[admin].g_flCloseAreasOnly, value, -1.0, 99999.0);
			g_esFirePlayer[admin].g_iComboAbility = iGetKeyValue(subsection, MT_FIRE_SECTION, MT_FIRE_SECTION2, MT_FIRE_SECTION3, MT_FIRE_SECTION4, key, "ComboAbility", "Combo Ability", "Combo_Ability", "combo", g_esFirePlayer[admin].g_iComboAbility, value, -1, 1);
			g_esFirePlayer[admin].g_iHumanAbility = iGetKeyValue(subsection, MT_FIRE_SECTION, MT_FIRE_SECTION2, MT_FIRE_SECTION3, MT_FIRE_SECTION4, key, "HumanAbility", "Human Ability", "Human_Ability", "human", g_esFirePlayer[admin].g_iHumanAbility, value, -1, 2);
			g_esFirePlayer[admin].g_iHumanAmmo = iGetKeyValue(subsection, MT_FIRE_SECTION, MT_FIRE_SECTION2, MT_FIRE_SECTION3, MT_FIRE_SECTION4, key, "HumanAmmo", "Human Ammo", "Human_Ammo", "hammo", g_esFirePlayer[admin].g_iHumanAmmo, value, -1, 99999);
			g_esFirePlayer[admin].g_iHumanCooldown = iGetKeyValue(subsection, MT_FIRE_SECTION, MT_FIRE_SECTION2, MT_FIRE_SECTION3, MT_FIRE_SECTION4, key, "HumanCooldown", "Human Cooldown", "Human_Cooldown", "hcooldown", g_esFirePlayer[admin].g_iHumanCooldown, value, -1, 99999);
			g_esFirePlayer[admin].g_iHumanRangeCooldown = iGetKeyValue(subsection, MT_FIRE_SECTION, MT_FIRE_SECTION2, MT_FIRE_SECTION3, MT_FIRE_SECTION4, key, "HumanRangeCooldown", "Human Range Cooldown", "Human_Range_Cooldown", "hrangecooldown", g_esFirePlayer[admin].g_iHumanRangeCooldown, value, -1, 99999);
			g_esFirePlayer[admin].g_iHumanRockCooldown = iGetKeyValue(subsection, MT_FIRE_SECTION, MT_FIRE_SECTION2, MT_FIRE_SECTION3, MT_FIRE_SECTION4, key, "HumanRockCooldown", "Human Rock Cooldown", "Human_Rock_Cooldown", "hrockcooldown", g_esFirePlayer[admin].g_iHumanRockCooldown, value, -1, 99999);
			g_esFirePlayer[admin].g_flOpenAreasOnly = flGetKeyValue(subsection, MT_FIRE_SECTION, MT_FIRE_SECTION2, MT_FIRE_SECTION3, MT_FIRE_SECTION4, key, "OpenAreasOnly", "Open Areas Only", "Open_Areas_Only", "openareas", g_esFirePlayer[admin].g_flOpenAreasOnly, value, -1.0, 99999.0);
			g_esFirePlayer[admin].g_iRequiresHumans = iGetKeyValue(subsection, MT_FIRE_SECTION, MT_FIRE_SECTION2, MT_FIRE_SECTION3, MT_FIRE_SECTION4, key, "RequiresHumans", "Requires Humans", "Requires_Humans", "hrequire", g_esFirePlayer[admin].g_iRequiresHumans, value, -1, 32);
			...
			g_esFirePlayer[admin].g_flFireDamage = flGetKeyValue(subsection, MT_FIRE_SECTION, MT_FIRE_SECTION2, MT_FIRE_SECTION3, MT_FIRE_SECTION4, key, "FireDamage", "Fire Damage", "Fire_Damage", "damage", g_esFirePlayer[admin].g_flFireDamage, value, -1.0, 99999.0);
			...
			g_esFirePlayer[admin].g_flFireRangeChance = flGetKeyValue(subsection, MT_FIRE_SECTION, MT_FIRE_SECTION2, MT_FIRE_SECTION3, MT_FIRE_SECTION4, key, "FireRangeChance", "Fire Range Chance", "Fire_Range_Chance", "rangechance", g_esFirePlayer[admin].g_flFireRangeChance, value, -1.0, 100.0);
			g_esFirePlayer[admin].g_iFireRangeCooldown = iGetKeyValue(subsection, MT_FIRE_SECTION, MT_FIRE_SECTION2, MT_FIRE_SECTION3, MT_FIRE_SECTION4, key, "FireRangeCooldown", "Fire Range Cooldown", "Fire_Range_Cooldown", "rangecooldown", g_esFirePlayer[admin].g_iFireRangeCooldown, value, -1, 99999);
			g_esFirePlayer[admin].g_iFireRockBreak = iGetKeyValue(subsection, MT_FIRE_SECTION, MT_FIRE_SECTION2, MT_FIRE_SECTION3, MT_FIRE_SECTION4, key, "FireRockBreak", "Fire Rock Break", "Fire_Rock_Break", "rock", g_esFirePlayer[admin].g_iFireRockBreak, value, -1, 1);
			g_esFirePlayer[admin].g_flFireRockChance = flGetKeyValue(subsection, MT_FIRE_SECTION, MT_FIRE_SECTION2, MT_FIRE_SECTION3, MT_FIRE_SECTION4, key, "FireRockChance", "Fire Rock Chance", "Fire_Rock_Chance", "rockchance", g_esFirePlayer[admin].g_flFireRockChance, value, -1.0, 100.0);
			g_esFirePlayer[admin].g_iFireRockCooldown = iGetKeyValue(subsection, MT_FIRE_SECTION, MT_FIRE_SECTION2, MT_FIRE_SECTION3, MT_FIRE_SECTION4, key, "FireRockCooldown", "Fire Rock Cooldown", "Fire_Rock_Cooldown", "rockcooldown", g_esFirePlayer[admin].g_iFireRockCooldown, value, -1, 99999);
	...
	if (mode < 3 && type > 0)
	{
		if (special && specsection[0] != '\0')
		{
			...
			g_esFireSpecial[type].g_flFireDamage = flGetKeyValue(subsection, MT_FIRE_SECTION, MT_FIRE_SECTION2, MT_FIRE_SECTION3, MT_FIRE_SECTION4, key, "FireDamage", "Fire Damage", "Fire_Damage", "damage", g_esFireSpecial[type].g_flFireDamage, value, -1.0, 99999.0);
		...
		{
			g_esFireAbility[type].g_flCloseAreasOnly = flGetKeyValue(subsection, MT_FIRE_SECTION, MT_FIRE_SECTION2, MT_FIRE_SECTION3, MT_FIRE_SECTION4, key, "CloseAreasOnly", "Close Areas Only", "Close_Areas_Only", "closeareas", g_esFireAbility[type].g_flCloseAreasOnly, value, -1.0, 99999.0);
			g_esFireAbility[type].g_iComboAbility = iGetKeyValue(subsection, MT_FIRE_SECTION, MT_FIRE_SECTION2, MT_FIRE_SECTION3, MT_FIRE_SECTION4, key, "ComboAbility", "Combo Ability", "Combo_Ability", "combo", g_esFireAbility[type].g_iComboAbility, value, -1, 1);
			g_esFireAbility[type].g_iHumanAbility = iGetKeyValue(subsection, MT_FIRE_SECTION, MT_FIRE_SECTION2, MT_FIRE_SECTION3, MT_FIRE_SECTION4, key, "HumanAbility", "Human Ability", "Human_Ability", "human", g_esFireAbility[type].g_iHumanAbility, value, -1, 2);
			g_esFireAbility[type].g_iHumanAmmo = iGetKeyValue(subsection, MT_FIRE_SECTION, MT_FIRE_SECTION2, MT_FIRE_SECTION3, MT_FIRE_SECTION4, key, "HumanAmmo", "Human Ammo", "Human_Ammo", "hammo", g_esFireAbility[type].g_iHumanAmmo, value, -1, 99999);
			g_esFireAbility[type].g_iHumanCooldown = iGetKeyValue(subsection, MT_FIRE_SECTION, MT_FIRE_SECTION2, MT_FIRE_SECTION3, MT_FIRE_SECTION4, key, "HumanCooldown", "Human Cooldown", "Human_Cooldown", "hcooldown", g_esFireAbility[type].g_iHumanCooldown, value, -1, 99999);
			g_esFireAbility[type].g_iHumanRangeCooldown = iGetKeyValue(subsection, MT_FIRE_SECTION, MT_FIRE_SECTION2, MT_FIRE_SECTION3, MT_FIRE_SECTION4, key, "HumanRangeCooldown", "Human Range Cooldown", "Human_Range_Cooldown", "hrangecooldown", g_esFireAbility[type].g_iHumanRangeCooldown, value, -1, 99999);
			g_esFireAbility[type].g_iHumanRockCooldown = iGetKeyValue(subsection, MT_FIRE_SECTION, MT_FIRE_SECTION2, MT_FIRE_SECTION3, MT_FIRE_SECTION4, key, "HumanRockCooldown", "Human Rock Cooldown", "Human_Rock_Cooldown", "hrockcooldown", g_esFireAbility[type].g_iHumanRockCooldown, value, -1, 99999);
			g_esFireAbility[type].g_flOpenAreasOnly = flGetKeyValue(subsection, MT_FIRE_SECTION, MT_FIRE_SECTION2, MT_FIRE_SECTION3, MT_FIRE_SECTION4, key, "OpenAreasOnly", "Open Areas Only", "Open_Areas_Only", "openareas", g_esFireAbility[type].g_flOpenAreasOnly, value, -1.0, 99999.0);
			g_esFireAbility[type].g_iRequiresHumans = iGetKeyValue(subsection, MT_FIRE_SECTION, MT_FIRE_SECTION2, MT_FIRE_SECTION3, MT_FIRE_SECTION4, key, "RequiresHumans", "Requires Humans", "Requires_Humans", "hrequire", g_esFireAbility[type].g_iRequiresHumans, value, -1, 32);
			...
			g_esFireAbility[type].g_flFireDamage = flGetKeyValue(subsection, MT_FIRE_SECTION, MT_FIRE_SECTION2, MT_FIRE_SECTION3, MT_FIRE_SECTION4, key, "FireDamage", "Fire Damage", "Fire_Damage", "damage", g_esFireAbility[type].g_flFireDamage, value, -1.0, 99999.0);
			...
			g_esFireAbility[type].g_flFireRangeChance = flGetKeyValue(subsection, MT_FIRE_SECTION, MT_FIRE_SECTION2, MT_FIRE_SECTION3, MT_FIRE_SECTION4, key, "FireRangeChance", "Fire Range Chance", "Fire_Range_Chance", "rangechance", g_esFireAbility[type].g_flFireRangeChance, value, -1.0, 100.0);
			g_esFireAbility[type].g_iFireRangeCooldown = iGetKeyValue(subsection, MT_FIRE_SECTION, MT_FIRE_SECTION2, MT_FIRE_SECTION3, MT_FIRE_SECTION4, key, "FireRangeCooldown", "Fire Range Cooldown", "Fire_Range_Cooldown", "rangecooldown", g_esFireAbility[type].g_iFireRangeCooldown, value, -1, 99999);
			g_esFireAbility[type].g_iFireRockBreak = iGetKeyValue(subsection, MT_FIRE_SECTION, MT_FIRE_SECTION2, MT_FIRE_SECTION3, MT_FIRE_SECTION4, key, "FireRockBreak", "Fire Rock Break", "Fire_Rock_Break", "rock", g_esFireAbility[type].g_iFireRockBreak, value, -1, 1);
			g_esFireAbility[type].g_flFireRockChance = flGetKeyValue(subsection, MT_FIRE_SECTION, MT_FIRE_SECTION2, MT_FIRE_SECTION3, MT_FIRE_SECTION4, key, "FireRockChance", "Fire Rock Chance", "Fire_Rock_Chance", "rockchance", g_esFireAbility[type].g_flFireRockChance, value, -1.0, 100.0);
			g_esFireAbility[type].g_iFireRockCooldown = iGetKeyValue(subsection, MT_FIRE_SECTION, MT_FIRE_SECTION2, MT_FIRE_SECTION3, MT_FIRE_SECTION4, key, "FireRockCooldown", "Fire Rock Cooldown", "Fire_Rock_Cooldown", "rockcooldown", g_esFireAbility[type].g_iFireRockCooldown, value, -1, 99999);
...
#endif
{
	...
	g_esFirePlayer[tank].g_iTankTypeRecorded = apply ? MT_GetRecordedTankType(tank, type) : 0;
	g_esFirePlayer[tank].g_iTankType = apply ? type : 0;
	int iType = g_esFirePlayer[tank].g_iTankTypeRecorded;

	if (bIsSpecialInfected(tank, MT_CHECK_INDEX|MT_CHECK_INGAME))
	{
		g_esFireCache[tank].g_flCloseAreasOnly = flGetSubSettingValue(apply, bHuman, g_esFireTeammate[tank].g_flCloseAreasOnly, g_esFirePlayer[tank].g_flCloseAreasOnly, g_esFireSpecial[iType].g_flCloseAreasOnly, g_esFireAbility[iType].g_flCloseAreasOnly, 1);
		g_esFireCache[tank].g_iComboAbility = iGetSubSettingValue(apply, bHuman, g_esFireTeammate[tank].g_iComboAbility, g_esFirePlayer[tank].g_iComboAbility, g_esFireSpecial[iType].g_iComboAbility, g_esFireAbility[iType].g_iComboAbility, 1);
		g_esFireCache[tank].g_flFireChance = flGetSubSettingValue(apply, bHuman, g_esFireTeammate[tank].g_flFireChance, g_esFirePlayer[tank].g_flFireChance, g_esFireSpecial[iType].g_flFireChance, g_esFireAbility[iType].g_flFireChance, 1);
		g_esFireCache[tank].g_flFireDamage = flGetSubSettingValue(apply, bHuman, g_esFireTeammate[tank].g_flFireDamage, g_esFirePlayer[tank].g_flFireDamage, g_esFireSpecial[iType].g_flFireDamage, g_esFireAbility[iType].g_flFireDamage, 1);
		g_esFireCache[tank].g_flFireDeathChance = flGetSubSettingValue(apply, bHuman, g_esFireTeammate[tank].g_flFireDeathChance, g_esFirePlayer[tank].g_flFireDeathChance, g_esFireSpecial[iType].g_flFireDeathChance, g_esFireAbility[iType].g_flFireDeathChance, 1);
		g_esFireCache[tank].g_flFireRange = flGetSubSettingValue(apply, bHuman, g_esFireTeammate[tank].g_flFireRange, g_esFirePlayer[tank].g_flFireRange, g_esFireSpecial[iType].g_flFireRange, g_esFireAbility[iType].g_flFireRange, 1);
		g_esFireCache[tank].g_flFireRangeChance = flGetSubSettingValue(apply, bHuman, g_esFireTeammate[tank].g_flFireRangeChance, g_esFirePlayer[tank].g_flFireRangeChance, g_esFireSpecial[iType].g_flFireRangeChance, g_esFireAbility[iType].g_flFireRangeChance, 1);
		g_esFireCache[tank].g_flFireRockChance = flGetSubSettingValue(apply, bHuman, g_esFireTeammate[tank].g_flFireRockChance, g_esFirePlayer[tank].g_flFireRockChance, g_esFireSpecial[iType].g_flFireRockChance, g_esFireAbility[iType].g_flFireRockChance, 1);
		g_esFireCache[tank].g_iFireAbility = iGetSubSettingValue(apply, bHuman, g_esFireTeammate[tank].g_iFireAbility, g_esFirePlayer[tank].g_iFireAbility, g_esFireSpecial[iType].g_iFireAbility, g_esFireAbility[iType].g_iFireAbility, 1);
		g_esFireCache[tank].g_iFireCooldown = iGetSubSettingValue(apply, bHuman, g_esFireTeammate[tank].g_iFireCooldown, g_esFirePlayer[tank].g_iFireCooldown, g_esFireSpecial[iType].g_iFireCooldown, g_esFireAbility[iType].g_iFireCooldown, 1);
		g_esFireCache[tank].g_iFireDeath = iGetSubSettingValue(apply, bHuman, g_esFireTeammate[tank].g_iFireDeath, g_esFirePlayer[tank].g_iFireDeath, g_esFireSpecial[iType].g_iFireDeath, g_esFireAbility[iType].g_iFireDeath, 1);
		g_esFireCache[tank].g_iFireEffect = iGetSubSettingValue(apply, bHuman, g_esFireTeammate[tank].g_iFireEffect, g_esFirePlayer[tank].g_iFireEffect, g_esFireSpecial[iType].g_iFireEffect, g_esFireAbility[iType].g_iFireEffect, 1);
		g_esFireCache[tank].g_iFireHit = iGetSubSettingValue(apply, bHuman, g_esFireTeammate[tank].g_iFireHit, g_esFirePlayer[tank].g_iFireHit, g_esFireSpecial[iType].g_iFireHit, g_esFireAbility[iType].g_iFireHit, 1);
		g_esFireCache[tank].g_iFireHitMode = iGetSubSettingValue(apply, bHuman, g_esFireTeammate[tank].g_iFireHitMode, g_esFirePlayer[tank].g_iFireHitMode, g_esFireSpecial[iType].g_iFireHitMode, g_esFireAbility[iType].g_iFireHitMode, 1);
		g_esFireCache[tank].g_iFireMessage = iGetSubSettingValue(apply, bHuman, g_esFireTeammate[tank].g_iFireMessage, g_esFirePlayer[tank].g_iFireMessage, g_esFireSpecial[iType].g_iFireMessage, g_esFireAbility[iType].g_iFireMessage, 1);
		g_esFireCache[tank].g_iFireRangeCooldown = iGetSubSettingValue(apply, bHuman, g_esFireTeammate[tank].g_iFireRangeCooldown, g_esFirePlayer[tank].g_iFireRangeCooldown, g_esFireSpecial[iType].g_iFireRangeCooldown, g_esFireAbility[iType].g_iFireRangeCooldown, 1);
		g_esFireCache[tank].g_iFireRockBreak = iGetSubSettingValue(apply, bHuman, g_esFireTeammate[tank].g_iFireRockBreak, g_esFirePlayer[tank].g_iFireRockBreak, g_esFireSpecial[iType].g_iFireRockBreak, g_esFireAbility[iType].g_iFireRockBreak, 1);
		g_esFireCache[tank].g_iFireRockCooldown = iGetSubSettingValue(apply, bHuman, g_esFireTeammate[tank].g_iFireRockCooldown, g_esFirePlayer[tank].g_iFireRockCooldown, g_esFireSpecial[iType].g_iFireRockCooldown, g_esFireAbility[iType].g_iFireRockCooldown, 1);
		g_esFireCache[tank].g_iFireSight = iGetSubSettingValue(apply, bHuman, g_esFireTeammate[tank].g_iFireSight, g_esFirePlayer[tank].g_iFireSight, g_esFireSpecial[iType].g_iFireSight, g_esFireAbility[iType].g_iFireSight, 1);
		g_esFireCache[tank].g_iHumanAbility = iGetSubSettingValue(apply, bHuman, g_esFireTeammate[tank].g_iHumanAbility, g_esFirePlayer[tank].g_iHumanAbility, g_esFireSpecial[iType].g_iHumanAbility, g_esFireAbility[iType].g_iHumanAbility, 1);
		g_esFireCache[tank].g_iHumanAmmo = iGetSubSettingValue(apply, bHuman, g_esFireTeammate[tank].g_iHumanAmmo, g_esFirePlayer[tank].g_iHumanAmmo, g_esFireSpecial[iType].g_iHumanAmmo, g_esFireAbility[iType].g_iHumanAmmo, 1);
		g_esFireCache[tank].g_iHumanCooldown = iGetSubSettingValue(apply, bHuman, g_esFireTeammate[tank].g_iHumanCooldown, g_esFirePlayer[tank].g_iHumanCooldown, g_esFireSpecial[iType].g_iHumanCooldown, g_esFireAbility[iType].g_iHumanCooldown, 1);
		g_esFireCache[tank].g_iHumanRangeCooldown = iGetSubSettingValue(apply, bHuman, g_esFireTeammate[tank].g_iHumanRangeCooldown, g_esFirePlayer[tank].g_iHumanRangeCooldown, g_esFireSpecial[iType].g_iHumanRangeCooldown, g_esFireAbility[iType].g_iHumanRangeCooldown, 1);
		g_esFireCache[tank].g_iHumanRockCooldown = iGetSubSettingValue(apply, bHuman, g_esFireTeammate[tank].g_iHumanRockCooldown, g_esFirePlayer[tank].g_iHumanRockCooldown, g_esFireSpecial[iType].g_iHumanRockCooldown, g_esFireAbility[iType].g_iHumanRockCooldown, 1);
		g_esFireCache[tank].g_flOpenAreasOnly = flGetSubSettingValue(apply, bHuman, g_esFireTeammate[tank].g_flOpenAreasOnly, g_esFirePlayer[tank].g_flOpenAreasOnly, g_esFireSpecial[iType].g_flOpenAreasOnly, g_esFireAbility[iType].g_flOpenAreasOnly, 1);
		g_esFireCache[tank].g_iRequiresHumans = iGetSubSettingValue(apply, bHuman, g_esFireTeammate[tank].g_iRequiresHumans, g_esFirePlayer[tank].g_iRequiresHumans, g_esFireSpecial[iType].g_iRequiresHumans, g_esFireAbility[iType].g_iRequiresHumans, 1);
	}
	else
	{
		g_esFireCache[tank].g_flCloseAreasOnly = flGetSettingValue(apply, bHuman, g_esFirePlayer[tank].g_flCloseAreasOnly, g_esFireAbility[iType].g_flCloseAreasOnly, 1);
		g_esFireCache[tank].g_iComboAbility = iGetSettingValue(apply, bHuman, g_esFirePlayer[tank].g_iComboAbility, g_esFireAbility[iType].g_iComboAbility, 1);
		g_esFireCache[tank].g_flFireChance = flGetSettingValue(apply, bHuman, g_esFirePlayer[tank].g_flFireChance, g_esFireAbility[iType].g_flFireChance, 1);
		g_esFireCache[tank].g_flFireDamage = flGetSettingValue(apply, bHuman, g_esFirePlayer[tank].g_flFireDamage, g_esFireAbility[iType].g_flFireDamage, 1);
		g_esFireCache[tank].g_flFireDeathChance = flGetSettingValue(apply, bHuman, g_esFirePlayer[tank].g_flFireDeathChance, g_esFireAbility[iType].g_flFireDeathChance, 1);
		g_esFireCache[tank].g_flFireRange = flGetSettingValue(apply, bHuman, g_esFirePlayer[tank].g_flFireRange, g_esFireAbility[iType].g_flFireRange, 1);
		g_esFireCache[tank].g_flFireRangeChance = flGetSettingValue(apply, bHuman, g_esFirePlayer[tank].g_flFireRangeChance, g_esFireAbility[iType].g_flFireRangeChance, 1);
		g_esFireCache[tank].g_flFireRockChance = flGetSettingValue(apply, bHuman, g_esFirePlayer[tank].g_flFireRockChance, g_esFireAbility[iType].g_flFireRockChance, 1);
		g_esFireCache[tank].g_iFireAbility = iGetSettingValue(apply, bHuman, g_esFirePlayer[tank].g_iFireAbility, g_esFireAbility[iType].g_iFireAbility, 1);
		g_esFireCache[tank].g_iFireCooldown = iGetSettingValue(apply, bHuman, g_esFirePlayer[tank].g_iFireCooldown, g_esFireAbility[iType].g_iFireCooldown, 1);
		g_esFireCache[tank].g_iFireDeath = iGetSettingValue(apply, bHuman, g_esFirePlayer[tank].g_iFireDeath, g_esFireAbility[iType].g_iFireDeath, 1);
		g_esFireCache[tank].g_iFireEffect = iGetSettingValue(apply, bHuman, g_esFirePlayer[tank].g_iFireEffect, g_esFireAbility[iType].g_iFireEffect, 1);
		g_esFireCache[tank].g_iFireHit = iGetSettingValue(apply, bHuman, g_esFirePlayer[tank].g_iFireHit, g_esFireAbility[iType].g_iFireHit, 1);
		g_esFireCache[tank].g_iFireHitMode = iGetSettingValue(apply, bHuman, g_esFirePlayer[tank].g_iFireHitMode, g_esFireAbility[iType].g_iFireHitMode, 1);
		g_esFireCache[tank].g_iFireMessage = iGetSettingValue(apply, bHuman, g_esFirePlayer[tank].g_iFireMessage, g_esFireAbility[iType].g_iFireMessage, 1);
		g_esFireCache[tank].g_iFireRangeCooldown = iGetSettingValue(apply, bHuman, g_esFirePlayer[tank].g_iFireRangeCooldown, g_esFireAbility[iType].g_iFireRangeCooldown, 1);
		g_esFireCache[tank].g_iFireRockBreak = iGetSettingValue(apply, bHuman, g_esFirePlayer[tank].g_iFireRockBreak, g_esFireAbility[iType].g_iFireRockBreak, 1);
		g_esFireCache[tank].g_iFireRockCooldown = iGetSettingValue(apply, bHuman, g_esFirePlayer[tank].g_iFireRockCooldown, g_esFireAbility[iType].g_iFireRockCooldown, 1);
		g_esFireCache[tank].g_iFireSight = iGetSettingValue(apply, bHuman, g_esFirePlayer[tank].g_iFireSight, g_esFireAbility[iType].g_iFireSight, 1);
		g_esFireCache[tank].g_iHumanAbility = iGetSettingValue(apply, bHuman, g_esFirePlayer[tank].g_iHumanAbility, g_esFireAbility[iType].g_iHumanAbility, 1);
		g_esFireCache[tank].g_iHumanAmmo = iGetSettingValue(apply, bHuman, g_esFirePlayer[tank].g_iHumanAmmo, g_esFireAbility[iType].g_iHumanAmmo, 1);
		g_esFireCache[tank].g_iHumanCooldown = iGetSettingValue(apply, bHuman, g_esFirePlayer[tank].g_iHumanCooldown, g_esFireAbility[iType].g_iHumanCooldown, 1);
		g_esFireCache[tank].g_iHumanRangeCooldown = iGetSettingValue(apply, bHuman, g_esFirePlayer[tank].g_iHumanRangeCooldown, g_esFireAbility[iType].g_iHumanRangeCooldown, 1);
		g_esFireCache[tank].g_iHumanRockCooldown = iGetSettingValue(apply, bHuman, g_esFirePlayer[tank].g_iHumanRockCooldown, g_esFireAbility[iType].g_iHumanRockCooldown, 1);
		g_esFireCache[tank].g_flOpenAreasOnly = flGetSettingValue(apply, bHuman, g_esFirePlayer[tank].g_flOpenAreasOnly, g_esFireAbility[iType].g_flOpenAreasOnly, 1);
		g_esFireCache[tank].g_iRequiresHumans = iGetSettingValue(apply, bHuman, g_esFirePlayer[tank].g_iRequiresHumans, g_esFireAbility[iType].g_iRequiresHumans, 1);
	}
}
...
#endif
{
	...
	else if (StrEqual(name, "player_death"))
	{
		...
		{
			vFireRange(iTank, 1, 0, GetRandomFloat(0.1, 100.0));
			vRemoveFire(iTank);
		}
	}
...
#endif
{
	if (MT_IsTankSupported(tank, MT_CHECK_INDEX|MT_CHECK_INGAME|MT_CHECK_FAKECLIENT) && ((!MT_HasAdminAccess(tank) && !bHasAdminAccess(tank, g_esFireAbility[g_esFirePlayer[tank].g_iTankTypeRecorded].g_iAccessFlags, g_esFirePlayer[tank].g_iAccessFlags)) || g_esFireCache[tank].g_iHumanAbility == 0))
	{
		return;
	}

	if (MT_IsTankSupported(tank) && (!bIsInfected(tank, MT_CHECK_FAKECLIENT) || g_esFireCache[tank].g_iHumanAbility != 1) && MT_IsCustomTankSupported(tank) && g_esFireCache[tank].g_iFireAbility == 1 && g_esFireCache[tank].g_iComboAbility == 0)
	{
...
#endif
{
	...
	{
		if (bIsAreaNarrow(tank, g_esFireCache[tank].g_flOpenAreasOnly) || bIsAreaWide(tank, g_esFireCache[tank].g_flCloseAreasOnly) || MT_DoesTypeRequireHumans(g_esFirePlayer[tank].g_iTankType, tank) || (g_esFireCache[tank].g_iRequiresHumans > 0 && iGetHumanCount() < g_esFireCache[tank].g_iRequiresHumans) || (!MT_HasAdminAccess(tank) && !bHasAdminAccess(tank, g_esFireAbility[g_esFirePlayer[tank].g_iTankTypeRecorded].g_iAccessFlags, g_esFirePlayer[tank].g_iAccessFlags)))
		{
			return;
		}
		...
		if ((button & MT_SUB_KEY) && g_esFireCache[tank].g_iFireAbility == 1 && g_esFireCache[tank].g_iHumanAbility == 1)
		{
			...
			{
				case true: vFireAbility(tank, GetRandomFloat(0.1, 100.0));
...
#endif
{
	...
	{
		if (bIsInfected(tank, MT_CHECK_FAKECLIENT) && ((!MT_HasAdminAccess(tank) && !bHasAdminAccess(tank, g_esFireAbility[g_esFirePlayer[tank].g_iTankTypeRecorded].g_iAccessFlags, g_esFirePlayer[tank].g_iAccessFlags)) || g_esFireCache[tank].g_iHumanAbility == 0))
		{
			return;
		}
...
#endif
{
	vFireRange(tank, 1, 1, GetRandomFloat(0.1, 100.0));
}
...
#endif
{
	if (bIsAreaNarrow(tank, g_esFireCache[tank].g_flOpenAreasOnly) || bIsAreaWide(tank, g_esFireCache[tank].g_flCloseAreasOnly) || MT_DoesTypeRequireHumans(g_esFirePlayer[tank].g_iTankType, tank) || (g_esFireCache[tank].g_iRequiresHumans > 0 && iGetHumanCount() < g_esFireCache[tank].g_iRequiresHumans) || (MT_IsTankSupported(tank, MT_CHECK_INDEX|MT_CHECK_INGAME|MT_CHECK_FAKECLIENT) && ((!MT_HasAdminAccess(tank) && !bHasAdminAccess(tank, g_esFireAbility[g_esFirePlayer[tank].g_iTankTypeRecorded].g_iAccessFlags, g_esFirePlayer[tank].g_iAccessFlags)) || g_esFireCache[tank].g_iHumanAbility == 0)))
	{
		return;
	}
...
void vFireAbility(int tank, float random, int pos = -1)
{
	...
	if (!bIsInfected(tank, MT_CHECK_FAKECLIENT) || (g_esFirePlayer[tank].g_iAmmoCount < g_esFireCache[tank].g_iHumanAmmo && g_esFireCache[tank].g_iHumanAmmo > 0))
	{
		...
		{
			if (bIsSurvivor(iSurvivor, MT_CHECK_INGAME|MT_CHECK_ALIVE) && !MT_IsAdminImmune(iSurvivor, tank) && !bIsAdminImmune(iSurvivor, g_esFirePlayer[tank].g_iTankType, g_esFireAbility[g_esFirePlayer[tank].g_iTankTypeRecorded].g_iImmunityFlags, g_esFirePlayer[iSurvivor].g_iImmunityFlags))
			{
				GetClientAbsOrigin(iSurvivor, flSurvivorPos);
				if (GetVectorDistance(flTankPos, flSurvivorPos) <= flRange && bIsVisibleToPlayer(tank, iSurvivor, g_esFireCache[tank].g_iFireSight, .range = flRange))
				{
		...
		if (iSurvivorCount == 0)
		{
			if (bIsInfected(tank, MT_CHECK_FAKECLIENT) && g_esFireCache[tank].g_iHumanAbility == 1)
			{
				//MT_PrintToChat(tank, "%s %t", MT_TAG3, "FireHuman4");
			}
		}
	}
	else if (bIsInfected(tank, MT_CHECK_FAKECLIENT) && g_esFireCache[tank].g_iHumanAbility == 1)
	{
...
{
	if (bIsAreaNarrow(tank, g_esFireCache[tank].g_flOpenAreasOnly) || bIsAreaWide(tank, g_esFireCache[tank].g_flCloseAreasOnly) || MT_DoesTypeRequireHumans(g_esFirePlayer[tank].g_iTankType, tank) || (g_esFireCache[tank].g_iRequiresHumans > 0 && iGetHumanCount() < g_esFireCache[tank].g_iRequiresHumans) || (!MT_HasAdminAccess(tank) && !bHasAdminAccess(tank, g_esFireAbility[g_esFirePlayer[tank].g_iTankTypeRecorded].g_iAccessFlags, g_esFirePlayer[tank].g_iAccessFlags)) || MT_IsAdminImmune(survivor, tank) || bIsAdminImmune(survivor, g_esFirePlayer[tank].g_iTankType, g_esFireAbility[g_esFirePlayer[tank].g_iTankTypeRecorded].g_iImmunityFlags, g_esFirePlayer[survivor].g_iImmunityFlags))
	{
		return;
	}
	...
	{
		if (!bIsInfected(tank, MT_CHECK_FAKECLIENT) || (flags & MT_ATTACK_CLAW) || (flags & MT_ATTACK_MELEE) || (g_esFirePlayer[tank].g_iAmmoCount < g_esFireCache[tank].g_iHumanAmmo && g_esFireCache[tank].g_iHumanAmmo > 0))
		{
			if (random <= chance)
			{
				...
				if ((flags & MT_ATTACK_RANGE) && (g_esFirePlayer[tank].g_iRangeCooldown == -1 || g_esFirePlayer[tank].g_iRangeCooldown <= iTime))
				{
					...
					iCooldown = (bIsInfected(tank, MT_CHECK_FAKECLIENT) && g_esFireCache[tank].g_iHumanAbility == 1 && g_esFirePlayer[tank].g_iAmmoCount < g_esFireCache[tank].g_iHumanAmmo && g_esFireCache[tank].g_iHumanAmmo > 0) ? g_esFireCache[tank].g_iHumanRangeCooldown : iCooldown;
				...
				else if (((flags & MT_ATTACK_CLAW) || (flags & MT_ATTACK_MELEE)) && (g_esFirePlayer[tank].g_iCooldown == -1 || g_esFirePlayer[tank].g_iCooldown <= iTime))
				{
					...
					iCooldown = (bIsInfected(tank, MT_CHECK_FAKECLIENT) && g_esFireCache[tank].g_iHumanAbility == 1) ? g_esFireCache[tank].g_iHumanCooldown : iCooldown;
			...
			{
				if (bIsInfected(tank, MT_CHECK_FAKECLIENT) && g_esFireCache[tank].g_iHumanAbility == 1 && !g_esFirePlayer[tank].g_bFailed)
				{
		...
		else if (bIsInfected(tank, MT_CHECK_FAKECLIENT) && g_esFireCache[tank].g_iHumanAbility == 1 && !g_esFirePlayer[tank].g_bNoAmmo)
		{
...
void vFireRockBreak2(int tank, int rock, float random, int pos = -1)
{
	...
	if (random <= flChance)
	{
		int iTime = GetTime(), iCooldown = (bIsInfected(tank, MT_CHECK_FAKECLIENT) && g_esFireCache[tank].g_iHumanAbility == 1) ? g_esFireCache[tank].g_iHumanRockCooldown : g_esFireCache[tank].g_iFireRockCooldown;
...
void tTimerFireCombo2(Handle timer, DataPack pack)
{
	...
	if ((g_esFireCache[iTank].g_iFireHitMode == 0 || g_esFireCache[iTank].g_iFireHitMode == 1) && (bIsSpecialInfected(iTank) || StrEqual(sClassname[7], "tank_claw") || StrEqual(sClassname, "tank_rock")))
	{
